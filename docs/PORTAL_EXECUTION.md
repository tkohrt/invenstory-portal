...
- [~] Phase 4 — CORE DONE (2026-07-29): real Supabase Auth. Mock cookie REPLACED. @supabase/ssr userClient() carries user session -> RLS enforces tenancy (service-role scaffold RETIRED from data.ts; db.ts now only ingestion + identity resolution). 4 auth users created+linked to app_user.auth_id, passwords in /tmp/user-passwords.txt (-> password manager, NOT git). proxy.ts route protection (Next16 middleware). /auth/callback handles ?code= AND ?token_hash= (playbook #11). Password reset (email-agnostic notice), update-password page. Redirect allowlist + site_url set via mgmt API. NEW PROBE SUITE scripts/probe-auth.mjs: signs in as REAL client Lili, 8/8 PASS incl. cannot-read/forge/leak KHAI docs+cards+chunks+audit_log (the logged-in-attacker test). Anon suite still 12/12. RESEND: account created, send-only API key stored (Vercel all targets + .env.local). Supabase custom SMTP CONFIGURED via mgmt API (smtp.resend.com:465, user=resend, sender noreply@forgranted.com). PENDING: forgranted.com domain verification — send-only key can't manage domains via API, so Tyler adds the domain in the Resend DASHBOARD (generates DNS records) and adds them to GoDaddy. Auth email delivery activates on verification; until then admin-created passwords work. TOTP MFA: enable at checkpoint (needs a real authenticator enrollment). Awaiting Tyler checkpoint (live journey + probe review).
